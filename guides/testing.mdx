---
title: Testing Guide
description: Learn how to test resolvers, workflows, and executors using Vitest with unit tests, mocks, and E2E testing strategies
---
...
```typescript src/resolver/simple.test.ts
...
```typescript src/resolver/simple.ts
...
```typescript src/resolver/mockTailordb.test.ts
...
```typescript src/resolver/decrementUserAge.ts
import { createResolver, t } from "@tailor-platform/sdk";
...
```typescript src/resolver/decrementUserAge.test.ts
...
```typescript src/workflow/calculation.test.ts
...
```typescript e2e/resolver.test.ts
...
```typescript e2e/workflow.test.ts
import { randomUUID } from "node:crypto";
...
- **Unit tests**: Place alongside source files (`src/resolver/*.test.ts`)
...
<Warning>
- Always restore mocks with `vi.restoreAllMocks()` or `vi.unstubAllGlobals()` in `afterEach` or `afterAll`
- E2E tests require unique identifiers (use `randomUUID()`) to avoid conflicts
- Remember to clean up test data after E2E tests when possible
</Warning>

## Next Steps

- Explore the [testing template](https://github.com/tailor-platform/templates/tree/main/testing) for complete examples
- Learn about [Resolvers](/services/resolver) for creating custom GraphQL operations
- Understand [Workflows](/services/workflow) for job orchestration
